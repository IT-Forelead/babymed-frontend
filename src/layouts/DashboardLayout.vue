...
const isOpen = computed(() =>
  useSidebarStore().isOpenSidebar
)

const isShowCloseSidebarBtn = computed(() =>
  useModalStore().isModalOpen || useModalStore().isOpenPaymentStatusChangerModal || useModalStore().isOpenAddVisitModal ||
  useModalStore().isOpenOperationExpenseInfo || useModalStore().isOpenPrintModal || useModalStore().isOpenPrintPdfModal ||
  useModalStore().isOpenAddServiceModal || useModalStore().isOpenEditServiceModal || useModalStore().isOpenDeleteAlertModal ||
  useModalStore().isOpenAddUserModal || useModalStore().isOpenAddRoleModal
)
</script>
<template>
  <div class="flex h-screen overflow-y-hidden bg-white relative">
    ...
    <div v-if="!isShowCloseSidebarBtn" @click="useSidebarStore().toggleSidebar()"
      :class="isOpen ? 'left-72' : 'left-12'"
