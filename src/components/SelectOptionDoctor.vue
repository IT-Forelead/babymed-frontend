...
<template>
  <div class="select-none">
    <label ref="dropdown" class="flex items-center w-full relative">
      ...
      <div @click="useDropStore().openDoctorDropDown()" v-else class="border-none bg-gray-100 py-2 w-full text-lg rounded-r-lg cursor-pointer text-gray-500 pl-2">Select</div>
      ...
      <TimesIcon @click="clearSelectedOptionData()" v-if="selectedOption" class="absolute shadow right-2.5 z-10 cursor-pointer bg-gray-500 hover:bg-gray-600 text-white rounded-full p-1" />
      <div v-if="useDropStore().isOpenDoctorDropDown" class="absolute p-2 z-20 top-12 max-h-56 overflow-auto w-full bg-gray-100 rounded-lg divide-y">
