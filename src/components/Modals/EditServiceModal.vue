...
watch(
  ...
  (data) => {
    if (data) {
      useDropStore().setSelectServiceTypeOption(serviceTypes.value.filter(st => st?.id === data?.serviceTypeId)[0])
...
<template>
  <div v-if="useModalStore().isOpenEditServiceModal" class="overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 backdrop-blur bg-gray-900/75 w-full max-h-screen md:inset-0 md:h-full">
