...
<template>
  <div v-if="useModalStore().isOpenDeleteAlertModal"
    ...
    <div class="relative max-w-xl p-4 w-full h-full md:h-auto left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2">
      <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
        ...
        <div v-if="selectedService?.id" class="py-4 px-6 text-center space-y-6">
          ...
          <button @click="deleteService(selectedService?.id)" type="button"
        ...
        <div v-if="selectedOperationService?.operationService?.id" class="py-4 px-6 text-center space-y-6">
          <div>
            ...
            <div class="flex items-center space-x-2">
              <div class="text-base text-gray-500">{{ $t('servicePrice') }}:</div>
              <div class="text-gray-700 text-sm font-bold italic">{{
    useMoneyFormatter(selectedOperationService?.service?.price) }}</div>
            </div>
          </div>
          <h3
            class="flex items-center justify-center mb-5 text-lg font-normal text-red-500 dark:text-gray-400 text-center">
          ...
          <button @click="deleteOperationService(selectedOperationService?.operationService?.id)" type="button"
            class="text-white bg-red-600 hover:bg-red-800 focus:ring-4 focus:outline-none focus:ring-red-300 dark:focus:ring-red-800 font-medium rounded-lg text-sm inline-flex items-center px-5 py-2.5 text-center mr-2">
            {{ $t('yesImSure') }}
          </button>
          <button @click="closeModal()" type="button"
        ...
        <div v-if="selectedPatient?.patient?.id" class="py-4 px-6 text-center space-y-6">
          <div>
            <div class="flex items-center space-x-2">
              ...
              <div class="text-lg text-gray-700">{{ selectedPatient?.patient?.firstname + ' ' +
    selectedPatient?.patient?.lastname }}</div>
            </div>
            <div class="flex items-center space-x-2">
              <div class="text-base text-gray-500">{{ $t('phone') }}:</div>
            ...
            <div class="flex items-center space-x-2">
              ...
              <div class="text-lg text-gray-700">{{ moment(selectedPatient?.patient?.birthday).format('DD/MM/YYYY') }}
              </div>
            </div>
            <div class="flex items-center space-x-2">
            ...
            <div class="flex items-center space-x-2">
              <div class="text-base text-gray-500">{{ $t('createdAt') }}:</div>
              <div class="text-lg text-gray-700">
                {{ moment(selectedPatient?.patient?.createdAt).format('DD/MM/YYYY H:mm') }}
              </div>
            </div>
          </div>
          <h3
          ...
          <button @click="deletePatient(selectedPatient?.patient?.id)" type="button"
            class="text-white bg-red-600 hover:bg-red-800 focus:ring-4 focus:outline-none focus:ring-red-300 dark:focus:ring-red-800 font-medium rounded-lg text-sm inline-flex items-center px-5 py-2.5 text-center mr-2">
            {{ $t('yesImSure') }}
          </button>
          <button @click="closeModal()" type="button"
        ...
        <div v-if="selectedDoctorShare?.doctorShare?.id" class="py-4 px-6 text-center space-y-6">
          <div>
            <div class="flex items-center space-x-2">
              ...
              <div class="text-lg text-gray-700">{{ selectedDoctorShare?.user?.firstname + ' ' +
    selectedDoctorShare?.user?.lastname }}</div>
            </div>
            <div class="flex items-center space-x-2">
          ...
          <button @click="deleteDoctorShare(selectedDoctorShare?.doctorShare?.id)" type="button"
            class="text-white bg-red-600 hover:bg-red-800 focus:ring-4 focus:outline-none focus:ring-red-300 dark:focus:ring-red-800 font-medium rounded-lg text-sm inline-flex items-center px-5 py-2.5 text-center mr-2">
            {{ $t('yesImSure') }}
          </button>
          <button @click="closeModal()" type="button"
