...
<template>
  <div class="select-none">
    <label ref="dropdown" class="flex items-center w-full relative">
      ...
      <div v-if="selectedOption" class="border-none focus:ring-0 outline-0 bg-gray-100 w-full text-lg rounded-r-lg pl-2 py-2 capitalize" v-text="selectedOption?.customer?.firstname + ' ' + selectedOption?.customer?.lastname"></div>
      ...
      <div v-if="useDropStore().isOpenPatientDropDown" class="absolute p-2 z-20 top-12 max-h-56 overflow-auto w-full bg-gray-100 rounded-lg divide-y">
