...
<template>
  <div class="select-none">
    <label ref="dropdown" class="flex items-center w-full relative">
      ...
      <input type="text" v-model="searchPatient" v-if="useDropStore().isOpenPatientDropDown" class="relative w-full foucus:ring-0 focus:outline-none border-none rounded-r-lg bg-gray-100" :placeholder="'Enter patient data ...'" />
      ...
      <div @click="useDropStore().openPatientDropDown()" v-if="!useDropStore().isOpenPatientDropDown && !selectedOption && !(router.currentRoute.value.path === '/patients' || router.currentRoute.value.path === '/dashboard')" class="border-none bg-gray-100 py-2 w-full text-lg rounded-r-lg cursor-pointer text-gray-500 pl-2">{{ $t('select') }}</div>
      <ChevronRightIcon @click="useDropStore().openPatientDropDown()" v-if="!useDropStore().isOpenPatientDropDown && !selectedOption && !(router.currentRoute.value.path === '/patients' || router.currentRoute.value.path === '/dashboard')" class="absolute right-2.5 z-10 rotate-90 cursor-pointer text-gray-600" />
