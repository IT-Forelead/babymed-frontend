...
<template>
  <div class="select-none">
    <label ref="dropdown" class="flex items-center w-full relative">
      ...
      <div v-if="selectedOption" class="border-none focus:ring-0 outline-0 bg-gray-100 w-full text-lg rounded-r-lg pl-2 py-2 capitalize" v-text="selectedOption?.patient?.firstname + ' ' + selectedOption?.patient?.lastname"></div>
      <div @click="useDropStore().openPatientDropDown()" v-else class="border-none bg-gray-100 py-2 w-full text-lg rounded-r-lg cursor-pointer text-gray-500 pl-2">{{$t('select')}}</div>
      <ChevronRightIcon @click="useDropStore().openPatientDropDown()" v-if="!selectedOption" class="absolute right-2.5 z-10 rotate-90 cursor-pointer text-gray-600" />
      ...
      <div v-if="useDropStore().isOpenPatientDropDown" class="absolute p-2 z-20 top-12 shadow max-h-56 overflow-auto w-full bg-gray-100 rounded-lg divide-y">
