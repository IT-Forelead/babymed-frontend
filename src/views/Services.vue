        ...
        <div class="space-y-5">
          <label for="serviceName">
            ...
            <input v-model="serviceData.name"
              class="text-gray-500 mb-3 border-none bg-gray-100 rounded-lg w-full text-lg" type="text" id="serviceName"
          ...
          <label for="servicePrice">
            {{ $t('servicePrice') }}
            <money3 v-model="serviceData.price" v-bind="moneyConf" id="servicePrice"
              class="border-none text-right text-gray-500 bg-gray-100 rounded-lg w-full text-lg"> </money3>
          </label>
          <div @click="submitServiceData()"
            :class="isLoading ? 'bg-gray-600' : 'bg-gray-900 hover:bg-gray-800 cursor-pointer'"
            class="w-full py-3 text-white rounded-lg flex items-center justify-center">
            <svg v-if="isLoading"
