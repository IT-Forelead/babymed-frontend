...
const loadExpenses = async ($state) => {
  ...
  if (total.value !== 0 && total.value / 30 + additional >= page) {
    AxiosService.post("/checkup-expense/report", {page: page, limit: 30}, {headers: authHeader()})
        .then((result) => {
  ...
  <div class="bg-white rounded-lg w-full p-5">
    <div class="flex items-center justify-between">
      <div class="flex items-center space-x-3">
        ...
        <div>|</div>
        <div @click="openThirdTab()"
             :class="useTabStore().isOpenThirdTab ? 'bg-lime-400' : 'bg-gray-200 hover:bg-gray-400 cursor-pointer transition-all duration-300 hover:scale-105'"
             class="rounded-lg p-1.5 px-3 flex items-center">
      ...
      <div class="flex items-center space-x-3">
        ...
        <div v-if="useTabStore().isOpenSecondTab" class="flex items-center space-x-3">
          ...
          <!-- <div @click="useModalStore().openDailyExpenseModal()" class="bg-black text-white rounded-xl p-2 px-4 cursor-pointer hover:bg-black/75">
            <p class="text-base">{{ $t('report') }}</p>
          </div> -->
        </div>
      </div>
    </div>
