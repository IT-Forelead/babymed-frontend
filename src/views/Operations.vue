...
const loadExpenses = async ($state) => {
  ...
  if (total.value !== 0 && total.value / 30 + additional >= page) {
    AxiosService.post("/operation-expense/operations", {page: page, limit: 10}, {headers: authHeader()})
        .then((result) => {
...
<template>
  <div class="bg-white rounded-lg w-full p-5">
    <div class="flex items-center justify-between">
      <div class="flex items-center space-x-3">
        ...
        <div @click="openSecondTab()"
             ...
             class="rounded-lg p-1.5 px-3 flex items-center">
          <ServicesIcon class="w-5 h-5 mr-1"/>
          {{ $t('operationServices') }}
        </div>
      </div>
      <div class="flex items-center space-x-3">
        <div class="relative" ref="dropdown">
          <div v-if="useTabStore().isOpenFirstTab" @click="useModalStore().toggleFilterBy()"
          ...
          <div v-if="useModalStore().isOpenFilterBy"
            ...
            <div>
              ...
              <select v-model="filterData.serviceId" class="border-none text-gray-500 bg-gray-100 rounded-lg w-full">
                ...
                <option v-for="(service, idx) in operationServices" :key="idx"
                        :value="service?.operationService?.serviceId">
    ...
    <div v-if="useTabStore().isOpenSecondTab" class="grid grid- grid-cols-3 mt-5 gap-8">
      <div class="max-h-[75vh] overflow-auto xxl:overflow-hidden col-span-2">
